...
public static class ConnectionVoxelContainer
{
    ...
    {
        if (container[c.GridX, c.GridY, c.GridZ].Contains(c))
            container[c.GridX, c.GridY, c.GridZ].Remove(c);
        else
    ...
    {
        if (container[_x, _y, _z].Contains(c))
            container[_x, _y, _z].Remove(c);
        else
            Debug.Log("Connection Not Found");
    }
    ...
    public static List<Connection> RevealCloseConnections(Connection c)
    {
        ...
        if (c != null && c.GridX != -1 && c.GridY != -1 && c.GridZ != -1)
        {
            ...
            {
                //Debug.Log(string.Format("i: {0}, j: {1}, k: {2}", i, j, k));
